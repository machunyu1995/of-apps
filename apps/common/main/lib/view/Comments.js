...
define([
    ...
    Common.Views.Comments = Common.UI.BaseView.extend(_.extend({
        ...
        isSystemComment: function (username) {
            return username.endsWith('__SYS_CRM_HIDDEN__')
        },

        getSystemCommentName: function (username) {
          const name = username.replace('__SYS_CRM_HIDDEN__', '')
          switch (name) {
            case '新增':
              return Common.Locale.get('textNew', {name: 'Common.Views.Comments', default: 'Add'})
            case '修改':
              return Common.Locale.get('textRevise', {name: 'Common.Views.Comments', default: 'Revise'})
            case '删除':
              return Common.Locale.get('textDelete', {name: 'Common.Views.Comments', default: 'Delete'})
            default:
              return username.replace('__SYS_CRM_HIDDEN__', '')
          }
        },
