...
define([
    ...
    Common.Views.ReviewPopover = Common.UI.Window.extend(_.extend({
        ...
        showComments: function (animate, loadText, focus, showText) {
            if(this.api.asc_getSysCommentVisible()){
                // 当点击调转时
                this.showInsertTip()
                if(!this.api.asc_isViewMode()){
                    return
                }
            }else{
                let SystemComments = this.api.GetDocument().GetSystemComments();
                // 全部是 系统批注  不显示
                this.insertTipShow();
                let everyIsSys = this.commentsStore.every(x=> SystemComments.includes(x.get('uid')));
                if(everyIsSys){
                    return
                }
            }
        ...
        getSdkPanelHeight(){
            let sdkPanelTop = $('#id_panel_top');
            if (sdkPanelTop.length) {
                return (sdkPanelTop.css('display') !== 'none') ? sdkPanelTop.height() : 0;
            } else {
                sdkPanelTop = $('#ws-h-scrollbar');
                if (sdkPanelTop.length) {
                    return (sdkPanelTop.css('display') !== 'none') ? sdkPanelTop.height() : 0;
        ...
        getInsertTip: function(){
            return $('<div style="width: 25px; height: 30px; background-color: rgb(124, 123, 241); position: absolute; z-index: 100; top: 186px; left: 1225px; display: none; border-radius: 5px;"><svg class="icon" style="width: 100%;height: 100%;vertical-align: middle;fill: currentColor;overflow: hidden;" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="17529"><path d="M512 63.993c35.2 0 64 28.8 64 64v768c0 35.2-28.8 64-64 64s-64-28.8-64-64v-768c0-35.2 28.8-64 64-64z" fill="#FFFFFF" p-id="17530"></path><path d="M301.145 594.649L557.34 850.855c24.881 24.882 24.881 65.599 0 90.482-24.883 24.885-65.602 24.885-90.485 0L210.66 685.131c-24.881-24.882-24.881-65.599 0-90.482 24.883-24.884 65.602-24.884 90.485 0z" fill="#FFFFFF" p-id="17531"></path><path d="M722.855 594.649L466.66 850.855c-24.881 24.882-24.881 65.599 0 90.482 24.883 24.885 65.602 24.885 90.485 0L813.34 685.131c24.881-24.882 24.881-65.599 0-90.482-24.883-24.884-65.602-24.884-90.485 0z" fill="#FFFFFF" p-id="17532"></path></svg></div>')
        },

        renderInsertTip: function(){
            this.tipShow = false
            this.$tipDivTop = this.getInsertTip()
            $("#id_main").append(this.$tipDivTop);
        },

        showInsertTip: function(){
            this.$tipDivTop.css("display","block");
            this.tipShow = true
        },

        insertTipShow: function(){
            if(this.tipShow){
                this.$tipDivTop.css("display","block");
            }
        },

        hideInsertTip: function(){
            this.$tipDivTop.css("display","none");
            this.tipShow = false
        },

        insertTipHide: function(){
            if(this.tipShow){
                this.$tipDivTop.css("display","none");
            }
        },

        setInsertTipPosition: function(){
            var movePos = this.getCursorPosition();
            if (movePos) {
                let sdkPanelHeight = this.getSdkPanelHeight()
                // -6 优化展示
                this.$tipDivTop.css("top",(movePos[0][1] + sdkPanelHeight - 25 - 6)+'px');
                // 减去一半的宽度   
                this.$tipDivTop.css("left",(movePos[0][0] - 13)+'px');   
            }
        },
        ...
        getCursorPosition: function() {
            ...
            if (sdkPanelLeft.length)
                sdkPanelLeftWidth = (sdkPanelLeft.css('display') !== 'none') ? sdkPanelLeft.width() : 0;
            var x0 = p[0][0] + sdkPanelLeftWidth, y0 = p[0][1],
                x1 = p[2][0] + sdkPanelLeftWidth, y1 = p[2][1];
            var newDown = (y0>y1) ? [x0, y0] : [x1, y1],// try move down
                newUp = (y0<y1) ? [x0, y0] : [x1, y1]; // try move up
            return [newUp, newDown];
        },
